...
| GET `/notifications/stats` | `metrics.view` | Zählwerte `{total, unread, read, muted}` der eigenen Benachrichtigungen — `?kind=`/`?code=`/`?before=`/`?after=`/`?muted=1|0` grenzen die Grundmenge ein |
