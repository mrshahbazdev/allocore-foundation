...
Erinnerungen: `compliance:remind` (stündlich) — Unterweisungen, Prüfungen, Fristen, Feststellungen, Audit-Starts, Maßnahmen und GB-Reviews (Mail + Glocke; GBs gehen an den User mit der E-Mail der zugeordneten Person).
...
| `/metrics`, `/metrics/{metric}` | `metrics.view` | KPI-Snapshots aus `analytics:aggregate` (daily) — Counts je Modul + `compliance_rate` + `team_members` + Finanz-Summen `fin_revenue`, `fin_ebitda`, `fin_cashflow`, `fin_liquidity` + Audit-Kennzahlen `audits`, `audits_planned`, `audits_in_progress`, `audit_findings_open`, `audit_findings_overdue` |
| `/analytics/trends` | `metrics.view` | Layer 7: Trend je Metrik — letzter Wert, Vorgänger, `delta`, `direction` (up/down/flat/unknown) |
| `/insights` | `metrics.view` | Regelbasierte Hinweise (KI-Steuerung): `code`, `severity` (critical/warning/info), `message`, `data` — Codes: `tasks_overdue`, `tasks_due_soon`, `compliance_rate_low`, `high_risks_open`, `risk_reviews_overdue`, `deadlines_overdue`, `deadlines_due_soon`, `instructions_overdue`, `instructions_due_soon`, `inspections_overdue`, `inspections_due_soon`, `measures_overdue`, `orders_overdue`, `machines_in_maintenance`, `fin_negative_liquidity`, `leave_requests_pending`, `leave_active_today`, `tenders_open`, `tenders_deadline_soon`, `projects_overdue`, `projects_ending_soon`, `questions_open`, `measures_due_soon`, `orders_due_soon`, `strategies_overdue`, `investments_drawdown`, `participations_capital_need`, `tasks_unassigned`, `audit_findings_overdue`, `audit_findings_critical`, `audit_findings_due_soon`, `audit_findings_unassigned`, `audits_starting_soon`, `audits_overdue`, `audits_unassigned`, `measures_unassigned`, `deadlines_unassigned`, `inspections_unassigned`, `instructions_unassigned`, `orders_unassigned`, `projects_unassigned`, `all_clear` |
