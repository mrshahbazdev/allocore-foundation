...
| DELETE `/notifications` | `metrics.view` | Benachrichtigungen des Users löschen — `{deleted: n}`; ohne Filter alle, mit `?kind=`/ `?read=1` / `?unread=1` / `?code=` / `?muted=1` / `?before=YYYY-MM-DD` nur passende |
