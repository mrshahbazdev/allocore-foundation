...
| `/notifications` | `metrics.view` | Datenbank-Benachrichtigungen des Users (z. B. aus `compliance:remind`): `[{id, kind, title, due_at, read, created_at}]` — `limit` ≤ 50 (Default 10), `unread=1` nur ungelesene, `kind=` nach Art filtern |
