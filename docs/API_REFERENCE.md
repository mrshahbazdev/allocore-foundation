...
| GET `/notifications/stats` | `metrics.view` | Zählwerte `{total, unread, read, muted}` der eigenen Benachrichtigungen — `?kind=`/`?code=`/`?before=`/`?after=`/`?muted=1|0`/`?due_before=`/`?due_after=`/`?q=` grenzen die Grundmenge ein |
