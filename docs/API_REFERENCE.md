...
| GET `/notifications/stats` | `metrics.view` | Zählwerte `{total, unread, read, muted}` der eigenen Benachrichtigungen — `?kind=`/`?code=`/`?before=`/`?after=` grenzen die Grundmenge ein |
