...
class DataPlatformTest extends TestCase
{
    ...
    public function test_notifications_destroy_all_filters_by_muted(): void
    {
        $tenant = Tenant::create(['name' => 'DM GmbH']);
        $user = $this->acting($tenant);
        $this->putJson('/api/v1/me/notification-prefs', ['muted_kinds' => ['anmeldung']], ['X-Tenant' => $tenant->id])->assertOk();
        ...
        $this->deleteJson('/api/v1/notifications?muted=1', [], ['X-Tenant' => $tenant->id])
            ->assertOk()->assertExactJson(['deleted' => 1]);
    ...
    public function test_notifications_delete_read_honors_muted_filter(): void
    {
        ...
        $this->postJson('/api/v1/notifications/delete-read?muted=1', [], ['X-Tenant' => $tenant->id])
            ->assertOk()->assertExactJson(['deleted' => 1]);

        $this->assertSame(1, $user->notifications()->count());
    }

    public function test_notifications_delete_read_honors_kind_filter(): void
    {
        $tenant = Tenant::create(['name' => 'DR GmbH']);
        $user = $this->acting($tenant);

        $user->notify(new PasswordChangedAlert('x'));
        $n = new NewLoginAlert('1.2.3.4', null);
        $user->notify($n);
        $user->notifications()->update(['read_at' => now()]);

        $this->postJson('/api/v1/notifications/delete-read?kind=anmeldung', [], ['X-Tenant' => $tenant->id])
            ->assertOk()->assertExactJson(['deleted' => 1]);
    ...
    public function test_notifications_index_filters_by_code(): void
    {
        ...
        $user->notify(new CriticalInsight($tenant->id, 'demo_seed', 'Hinweis 1'));
        $user->notify(new CriticalInsight($tenant->id, 'orders_overdue', 'Hinweis 2'));

        $res = $this->getJson('/api/v1/notifications?code=demo_seed', ['X-Tenant' => $tenant->id])->assertOk()->json();
        $this->assertCount(1, $res);
        $this->assertSame('Hinweis 1', $res[0]['title']);
    }

    public function test_notifications_index_filters_by_q(): void
    {
        $tenant = Tenant::create(['name' => 'QS GmbH']);
        $user = $this->acting($tenant);

        $user->notify(new CriticalInsight($tenant->id, 'demo_seed', 'Frist läuft ab'));
        $user->notify(new CriticalInsight($tenant->id, 'orders_overdue', 'Auftrag überfällig'));
    ...
    public function test_notifications_read_all_honors_kind_filter(): void
    {
        ...
        $this->postJson('/api/v1/notifications/read-all?kind=anmeldung', [], ['X-Tenant' => $tenant->id])->assertOk();

        $this->getJson('/api/v1/notifications/unread-count', ['X-Tenant' => $tenant->id])
            ->assertOk()->assertJson(['count' => 1]);
        $this->getJson('/api/v1/notifications?unread=1', ['X-Tenant' => $tenant->id])
    ...
    public function test_notifications_read_all_honors_before_filter(): void
    {
        $tenant = Tenant::create(['name' => 'RB GmbH']);
        $user = $this->acting($tenant);
    ...
    public function test_delete_notifications_filtered(): void
    {
        $tenant = Tenant::create(['name' => 'N']);
        $user = $this->acting($tenant);
        $user->notify(new PasswordChangedAlert('a'));
        $user->notify(new NewLoginAlert('1.2.3.4', null));
        $user->notify(new NewLoginAlert('5.6.7.8', null));

        $this->deleteJson('/api/v1/notifications?kind=anmeldung', [], ['X-Tenant' => $tenant->id])
            ->assertOk()->assertJsonPath('deleted', 2);
        $this->assertSame(1, $user->notifications()->count());

        $user->notifications()->update(['read_at' => now()]);
        $this->deleteJson('/api/v1/notifications?read=1', [], ['X-Tenant' => $tenant->id])
